...
public struct DocumentTransformTool: LaicaiTool {
    ...
    public var functionDefinition: FunctionDefinition {
        FunctionDefinition(
            ...
            parameters: FunctionParameters(
                properties: [
                    "action": FunctionProperty(type: "string", description: "操作：workspace/inspect/prepare/apply/copy/verify/render。workspace 创建交付工作区；prepare 分块返回可编辑文本；apply 累积写回 translationsJSON；render 用 LibreOffice 导出 PDF；verify 检查输出文件。", enumValues: ["workspace", "inspect", "prepare", "apply", "copy", "verify", "render"]),
                    ...
                    "translationsJSON": FunctionProperty(type: "string", description: "apply 时使用的 JSON。支持对象 {\"part::index\":\"新文本\"} 或数组 [{\"id\":\"part::index\",\"text\":\"新文本\"}]；也支持用原文作为 key。"),
                    ...
                    "granularity": FunctionProperty(type: "string", description: "文本粒度：paragraph/text。pptx/docx 默认 paragraph，xlsx/xlsm 默认 text。", enumValues: ["paragraph", "text"])
                ],
                required: ["action"]
            )
        )
    ...
    public func execute(argumentsJSON: String, context: TaskContext) async throws -> ToolResult {
        ...
        guard ["workspace", "inspect", "prepare", "apply", "copy", "verify", "render"].contains(action) else {
        ...
        guard let rawSource = (params.sourcePath ?? params.path)?.trimmingCharacters(in: .whitespacesAndNewlines),
        ...
        if let securityError = await SecurityManager.shared.checkRead(path: sourcePath) {
        ...
        let outputPath = rawOutput.flatMap { $0.isEmpty ? nil : Self.resolvedPath($0, workspaceRoot: context.workspaceRoot) }
            ?? Self.defaultOutputPath(for: sourcePath)
        let readsExistingOutput = ["apply", "verify", "render"].contains(action)
            && FileManager.default.fileExists(atPath: outputPath)
        let sourceForRead = readsExistingOutput ? outputPath : sourcePath
        if sourceForRead != sourcePath, let securityError = await SecurityManager.shared.checkRead(path: sourceForRead) {
            return ToolResult(output: securityError, success: false, error: "security_denied")
        }
        let ext = (sourceForRead as NSString).pathExtension.lowercased()
        guard Self.supportedExtensions.contains(ext) else {
            return ToolResult(
                output: "document.transform 暂不支持 .\(ext.isEmpty ? "unknown" : ext)。目前支持 pptx/docx/xlsx/xlsm。",
                data: ["sourcePath": sourcePath, "extension": ext],
                success: false,
                error: "unsupported_file_type"
            )
        }
        let rawWorkflowPath = params.workflowPath?.trimmingCharacters(in: .whitespacesAndNewlines)
        let workflowPath = rawWorkflowPath.flatMap { $0.isEmpty ? nil : Self.resolvedPath($0, workspaceRoot: context.workspaceRoot) }
        ...
        let renderDir = rawRenderDir.flatMap { $0.isEmpty ? nil : Self.resolvedPath($0, workspaceRoot: context.workspaceRoot) }
            ?? (workflowPath as NSString).appendingPathComponent("rendered")
        let requestedGranularity = (params.granularity ?? "").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        let granularity = ["paragraph", "text"].contains(requestedGranularity)
            ? requestedGranularity
            : (["pptx", "docx"].contains(ext) ? "paragraph" : "text")

        if ["apply", "copy"].contains(action) {
            if let securityError = await SecurityManager.shared.checkWrite(path: outputPath) {
                return ToolResult(output: securityError, success: false, error: "security_denied")
            }
            if let dangerousError = DangerousOperationGuard.documentWriteViolation(path: outputPath, context: context) {
                return ToolResult(output: dangerousError, success: false, error: "dangerous_operation")
            }
        }
        if action == "workspace" {
            if let securityError = await SecurityManager.shared.checkWrite(path: workflowPath) {
                return ToolResult(output: securityError, success: false, error: "security_denied")
            }
            if let dangerousError = DangerousOperationGuard.documentWriteViolation(path: workflowPath, context: context) {
                return ToolResult(output: dangerousError, success: false, error: "dangerous_operation")
            }
        }
        if action == "render" {
            if let securityError = await SecurityManager.shared.checkWrite(path: renderDir) {
                return ToolResult(output: securityError, success: false, error: "security_denied")
            }
            if let dangerousError = DangerousOperationGuard.documentWriteViolation(path: renderDir, context: context) {
                return ToolResult(output: dangerousError, success: false, error: "dangerous_operation")
            }
        }
        ...
        do {
            let json = try Self.runPython(script: Self.pythonScript, input: payload)
            let summary = Self.summary(from: json, fallbackAction: action, outputPath: outputPath)
            let data = Self.resultData(from: json, sourcePath: sourceForRead, outputPath: outputPath, action: action)

            await AuditLog.shared.record(
                tool: name,
                input: "\(action) \(sourcePath)",
                output: summary,
                success: true
            )
...
def write_workflow(entries, media_count):
    ...
    with open(readme_path, 'w', encoding='utf-8') as fh:
        ...
        fh.write('This folder records a reproducible document-delivery run: source copy, working copy, scripts, cache, rendered output, and manifest.\\n')
