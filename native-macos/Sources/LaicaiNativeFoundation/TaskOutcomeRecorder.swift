...
public struct TaskOutcomeRecord {
    ...
    public var userFollowupCount: Int
...
public final class TaskOutcomeRecorder {
    ...
    public init(path: String? = nil) {
        ...
        self.path = directory.appendingPathComponent("outcomes.sqlite3").path
        migrate()
    }

    private func migrate() {
        withDatabase(()) { database in
            Self.ensureTaskOutcomesTable(on: database)
            Self.ensureToolOutcomesTable(on: database)
            Self.ensureExecutionTracesTable(on: database)
        }
    }
    ...
    private static func ensureTaskOutcomesTable(on database: OpaquePointer) {
        SQLiteSupport.exec(
            """
            ...
            """, on: database)
        SQLiteSupport.exec("CREATE INDEX IF NOT EXISTS idx_outcome_intent ON task_outcomes(intent);", on: database)
        SQLiteSupport.exec("CREATE INDEX IF NOT EXISTS idx_outcome_status ON task_outcomes(status);", on: database)
        SQLiteSupport.exec("CREATE INDEX IF NOT EXISTS idx_outcome_created ON task_outcomes(created_at);", on: database)
        SQLiteSupport.exec("CREATE INDEX IF NOT EXISTS idx_outcome_prompt ON task_outcomes(prompt_tag);", on: database)
        SQLiteSupport.exec("ALTER TABLE task_outcomes ADD COLUMN prompt_tag TEXT NOT NULL DEFAULT '';", on: database)
        SQLiteSupport.exec("ALTER TABLE task_outcomes ADD COLUMN user_rating INTEGER NOT NULL DEFAULT 0;", on: database)
        SQLiteSupport.exec("ALTER TABLE task_outcomes ADD COLUMN model_name TEXT NOT NULL DEFAULT '';", on: database)
        SQLiteSupport.exec("CREATE INDEX IF NOT EXISTS idx_outcome_model ON task_outcomes(model_name);", on: database)
    }

    private static func ensureToolOutcomesTable(on database: OpaquePointer) {
        SQLiteSupport.exec(
            """
            ...
            """, on: database)
        SQLiteSupport.exec("CREATE INDEX IF NOT EXISTS idx_tool_name ON tool_outcomes(tool_name);", on: database)
        SQLiteSupport.exec("CREATE INDEX IF NOT EXISTS idx_tool_model ON tool_outcomes(model_name);", on: database)
    }

    private static func ensureExecutionTracesTable(on database: OpaquePointer) {
        SQLiteSupport.exec(
            """
    ...
    public func record(_ record: TaskOutcomeRecord) {
        withDatabaseAsync { database in
            ...
            sqlite3BindTextSafe(stmt, 1, record.taskID)
            sqlite3BindTextSafe(stmt, 2, record.intent)
            sqlite3BindTextSafe(stmt, 3, record.routeLabel)
            sqlite3BindTextSafe(stmt, 4, record.executionMode)
            sqlite3_bind_int(stmt, 5, Int32(record.iterations))
            sqlite3BindTextSafe(stmt, 6, record.status.rawValue)
            sqlite3_bind_int(stmt, 7, record.hadFailure ? 1 : 0)
            sqlite3_bind_int(stmt, 8, record.wasCancelled ? 1 : 0)
            sqlite3_bind_int(stmt, 9, record.wasTruncated ? 1 : 0)
            sqlite3_bind_int(stmt, 10, Int32(record.toolCalls))
            sqlite3_bind_int(stmt, 11, Int32(record.toolFailures))
            sqlite3_bind_double(stmt, 12, record.durationSeconds)
            sqlite3_bind_int(stmt, 13, Int32(record.userFollowupCount))
            sqlite3BindTextSafe(stmt, 14, record.promptTag)
            sqlite3_bind_int(stmt, 15, Int32(record.userRating))
            sqlite3_bind_double(stmt, 16, now)
            sqlite3BindTextSafe(stmt, 17, record.modelName)
            sqlite3_step(stmt)
    ...
    public func rate(taskID: String, rating: Int) {
        withDatabaseAsync { database in
            ...
            sqlite3BindTextSafe(stmt, 2, taskID)
            sqlite3_step(stmt)
            sqlite3_finalize(stmt)
        }
    }
    ...
    public func recordToolOutcome(
        ...
        withDatabaseAsync { database in
            Self.ensureToolOutcomesTable(on: database)
            let sql =
                "INSERT INTO tool_outcomes (task_id, tool_name, model_name, success, duration_seconds, was_retry, created_at) VALUES (?, ?, ?, ?, ?, ?, ?);"
            var stmt: OpaquePointer?
            guard sqlite3_prepare_v2(database, sql, -1, &stmt, nil) == SQLITE_OK else { return }
            sqlite3BindTextSafe(stmt, 1, taskID)
            sqlite3BindTextSafe(stmt, 2, toolName)
    ...
    public func storeTrace(taskID: String, traceJSON: String) {
        withDatabaseAsync { database in
            ...
            guard sqlite3_prepare_v2(database, sql, -1, &stmt, nil) == SQLITE_OK else { return }
            sqlite3BindTextSafe(stmt, 1, taskID)
            sqlite3BindTextSafe(stmt, 2, traceJSON)
    ...
    public func avgIterations(intent: String, days: Int = 14) -> Double? {
        withReadOnlyDatabase(Double?.none) { database in
            let cutoff = Date().addingTimeInterval(-Double(days) * 24 * 3600).timeIntervalSince1970
            let sql =
                "SELECT AVG(iterations) FROM task_outcomes WHERE intent = ? AND status = 'completed' AND created_at > ? AND iterations > 0;"
            var stmt: OpaquePointer?
