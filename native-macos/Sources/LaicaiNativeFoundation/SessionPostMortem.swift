    ...
    private func detectEmptyFileWrites(steps: [TaskStep]) -> [Finding] {
        ...
        for (stepIndex, step) in steps.enumerated() {
            guard step.kind == .reviewRequest,
                Self.isFileChangeTool(step.toolName)
            else { continue }
    ...
    private func detectShellEncodingGarble(steps: [TaskStep]) -> [Finding] {
        ...
        for (stepIndex, step) in steps.enumerated() {
            guard step.kind == .toolResult,
                ["shell.exec", "verify.build", "shell_exec"].contains(step.toolName ?? "")
            else { continue }
    ...
    private func detectEditOnEmptyFile(steps: [TaskStep]) -> [Finding] {
        ...
        for (stepIndex, step) in steps.enumerated() {
            guard step.kind == .toolResult,
                ...
                step.isFailure,
                step.text.contains("所有编辑均失败") || step.text.contains("batch_failed")
            else { continue }
            ...
            for (key, calls) in grouped where calls.count >= 3 {
                ...
                findings.append(
                    Finding(
                        ...
                        evidence: calls.map {
                            EvidenceItem(stepIndex: $0.index, stepKind: "toolCall", toolName: $0.name, snippet: $0.pathParam)
                        },
                        suggestedFix: FixSuggestion(
    ...
    private func detectModelParseFailure(steps: [TaskStep]) -> [Finding] {
        ...
        for (stepIndex, step) in steps.enumerated() {
            guard step.kind == .error,
                step.text.contains("cannot parse") || step.text.contains("解析失败")
            else { continue }
            findings.append(
                Finding(
